...
public class testJuan implements Serializable { // No olvidarse del implements en ambos lados

    static ArrayList<Juan> discos;
    static Scanner sc = new Scanner(System.in);

    public static void crearColeccion() {
        discos = new ArrayList<Juan>();
    }

    // mock de datos
    public static void iniciaColeccion() {
        discos.add(new Juan("GASA41", "Wim Mertens", "Maximazing the Audience", "instrumental", 50));
        discos.add(new Juan("FGHQ64", "Metallica", "Black album", "hard rock", 46));
        discos.add(new Juan("TYUI89", "Supersubmarina", "Viento de cara", "pop rock", 42));
    }

    public static void mostrarColeccion() {
        for (Juan d : discos) { //Por cada disco
    ...
    private static void pideDisco() {
        ...
        if (res == 's') {
            discos.add(new Juan(codigoIn, autorIn, tituloIn, generoIn, duracionIn));
    ...
    public static void borrarDisco() {
        ...
        // metodo busqueda
        for (Juan dis : discos) { // por cada disco
    ...
    private static void cargarColeccion() throws ClassNotFoundException  {
        ...
        try (ObjectInputStream ois= new ObjectInputStream(new FileInputStream("misDiscos.bin"))){
            discos=(ArrayList<Juan>) ois.readObject();
            System.out.println("Coleccion de discos recuperada");
            for(Juan elem:discos) {
